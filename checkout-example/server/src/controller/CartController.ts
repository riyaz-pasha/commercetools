...
class CartController {
  ...
  async getActiveCart(req: Request, res: Response) {
    const options = getOptions(req.headers) // get the actual option
    const cartRepository = new CartRepository(options)
    let data = await cartRepository.getActiveCart()
    if (data.statusCode === 404) {
      console.error("‼️ No Active Cart found. Creating new cart");
      data = await cartRepository.createCart({
        ...req.body
      });
    }
