...
const createRandomPriceInAUD = () => {
  const randomNumber = Math.floor(Math.random() * (50000 - 10000 + 1)) + 10000;
  return {
    value: {
      currencyCode: 'AUD',
      centAmount: randomNumber,
    },
    country: 'AU',
  }
}
...
const toPrices = (stringPrices, customerGroups, channels) => {
  const allPrices = stringPrices
    .split(';')
    .map(toPrice(customerGroups, channels))
    .filter((x) => x)
  const auPrice = createRandomPriceInAUD()
  return [
    auPrice,
    ...allPrices,
  ]
